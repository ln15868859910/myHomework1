...
var maker = {
    ...
    methods: {
        ...
        setMaxSpan(date) {
            var startDate = new Date(
                this.parseDate(date).getTime() - parseInt(this.model.componentConfig.maxSpan - 1) * 24 * 3600 * 1000
            );
            return (
                this.getMoment(startDate, 'year') +
                '年' +
                this.getMoment(startDate, 'month') +
                '月' +
                this.getMoment(startDate, 'day') +
                '日'
            );
        },
        ...
        onFilterChange(data, list, type) {
            ...
            if (this.model.componentType == 'daterange' || this.model.componentType == 'date') {
                this.dispatch(
                    'consultFilter',
                    'single-change',
                    {
        ...
        initData() {
            ...
            if (modelList.componentType == 'daterange') {
                ...
                this.dispatch(
                    ...
                    'single-change',
                    {
                        ...
                        label: [
                            {
                                text: `开始时间：${me.dateFormat(
                                    defaultValueList[0],
                                    defaultFormat
                                )} - 结束时间：${me.dateFormat(defaultValueList[1], defaultFormat)}`,
            ...
            if (modelList.componentType == 'date') {
                //判断是否有初始值
                if (!modelList.componentConfig.value.length) {
                    return;
                }
                ...
                this.dispatch(
                    ...
                    'single-change',
                    {
                        ...
                        label: [
                            {
                                text: me.dateFormat(
                                    modelList.componentConfig.value[0],
                                    modelList.componentConfig.format
                                ),
    ...
    render(h) {
        ...
        if (me.model.componentType == 'daterange') {
            return h(DatePicker, {
                props: {
                    ...
                    options: {
                        shortcuts: me.model.componentConfig.optionList.length
                            ? this.setDateRangeShotcuts(me.model.componentConfig.optionList)
                            : []
                    },
                    ...
                    'on-change': function(list) {
                        if (list[0]) {
                            if (me.model.componentConfig.maxSpan) {
                                if (
                                    (me.parseDate(list[1]) - me.parseDate(list[0])) / 86400000 >
                                    me.model.componentConfig.maxSpan
                                ) {
                            ...
                            //保持当前model的value值与组件内部的value一致
                            me.model.componentConfig.value = [
                                me.dateFormat(list[0], 'YYYY-MM-DD'),
                                me.dateFormat(list[1], 'YYYY-MM-DD')
                            ];
                                    ...
                                    label: [
                                        {
                                            text: `开始时间：${me.dateFormat(
                                                list[0],
                                                me.model.componentConfig.format
                                            )} - 结束时间：${me.dateFormat(list[1], me.model.componentConfig.format)}`,
                                            value: [
                                                me.dateFormat(list[0], 'YYYY-MM-DD'),
                                                me.dateFormat(list[1], 'YYYY-MM-DD')
                                            ]
        ...
        } else if (me.model.componentType == 'date') {
            return h(DatePicker, {
                props: {
                    type: 'date',
                    value: me.model.componentConfig.value,
                ...
                on: {
                    ...
                    'on-change': function(list) {
                        if (list) {
                            ...
                            //保持当前model的value值与组件内部的value一致
                            me.model.componentConfig.value[0] = me.dateFormat(list, 'YYYY-MM-DD');
                            ...
                            me.dispatch(
                                ...
                                'single-change',
                                {
                                    ...
                                    label: [
                                        {
                                            text: me.dateFormat(list, me.model.componentConfig.format),
                                            value: [me.dateFormat(list, 'YYYY-MM-DD')]
                                        }
                                    ]
                                },
                                me.type
